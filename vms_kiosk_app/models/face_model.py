...
from vms_kiosk_app.utils import image_utils
    ...
    def __init__(self):
        ...
        for fname in config.FACE_CASCADE_MODELS:
            ...
            if not os.path.exists(cv2_model_file):
                raise Exception(
                    "CV2 model file %s does not exist", cv2_model_file)
            self.face_cascade_models.append(
                cv2.CascadeClassifier(cv2_model_file))

        logger.info("OpenCV cascade models initialized. # of models : %d", len(
            self.face_cascade_models))
